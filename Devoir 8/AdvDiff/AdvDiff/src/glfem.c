...
void glfemDrawCurve(double *x, double *y, int n) 
{
    ...
    bov_points_t* points = bov_points_new(coord,n,GL_STATIC_DRAW);
    bov_points_set_color(points,current_color);
    bov_points_set_width(points,0.002/zoom_init);

    bov_line_strip_draw(theCurrentWindow, points, 0, BOV_TILL_END);
    bov_points_delete(points);
    free(coord);
}
...
{
    glfemDrawCurve(x,y,n);
    glfemDrawNodes(x,y,n);
