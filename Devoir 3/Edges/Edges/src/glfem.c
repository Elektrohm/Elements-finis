...
void glfemDrawNodes(double *x, double *y, int n) {
  ...
  bov_points_set_width(points,0.0125/zoom_init);
...
void glfemPlotMesh(femMesh *theMesh){
    ...
    GLfloat (*data)[3] = malloc(sizeof(data[0][0])*3*nLocalNode*theMesh->nElem);
    ...
    for (i = 0; i < theMesh->nElem; ++i) {
        ...
        for (j=0; j < nLocalNode; ++j) {
            ...
            data[i*nLocalNode+j][2] = 0.0;}   
    }
    bov_points_t* points = bov_points_new_with_value(data,nLocalNode*theMesh->nElem,GL_STATIC_DRAW);
    
    bov_points_set_color(points, current_color);
    bov_points_set_width(points,current_line_width/zoom_init);
    bov_points_set_outline_width(points, current_line_width/zoom_init);
    bov_points_set_outline_color(points, current_color);
...
void glfemPlotEdges(femEdges *theEdges){
    ...
    GLfloat (*data)[3] = malloc(sizeof(data[0][0])*3*nLocalNode*theEdges->nEdge);
    
    for (i = 0; i < theEdges->nEdge; i++) {
        for (j=0; j < nLocalNode; ++j) {
            data[i*nLocalNode+j][0] = theMesh->X[theEdges->edges[i].node[j]];
            data[i*nLocalNode+j][1] = theMesh->Y[theEdges->edges[i].node[j]];
            data[i*nLocalNode+j][2] = 0.0;}   
    }
    bov_points_t* points = bov_points_new_with_value(data,nLocalNode*theEdges->nEdge,GL_STATIC_DRAW);
    ...
    bov_lines_draw(theCurrentWindow, points, 0, BOV_TILL_END);
...
void glfemPlotBnd(femEdges *theEdges){
    ...
    for (i = 0; i < theEdges->nEdge; i++) {
        if (theEdges->edges[i].elem[1] < 0) {
           ...
           for (j=0; j < nLocalNode; ++j) {
            	data[i*nLocalNode+j][0] = 0.0;
            	data[i*nLocalNode+j][1] = 0.0;
            	data[i*nLocalNode+j][2] = 0.0;}} 
    }
    bov_points_t* points = bov_points_new_with_value(data,nLocalNode*theEdges->nEdge,GL_STATIC_DRAW);
...
void glfemPlotSolution(femMesh* theMesh, double *u){
  ...
  for(int i = 0; i < theMesh->nElem; ++i){
    ...
    bov_points_set_width(points,0.0);
    bov_triangles_draw(theCurrentWindow, points, 0, BOV_TILL_END);
    bov_points_delete(points);
    free(data);}
}
